...
function buildMarketObjects(gammaMarkets, trades, { returnAllIfNoTrades = false } = {}) {
  ...
  if (marketsWithTrades.length > 0) {
    return marketsWithTrades;
  }

  if (returnAllIfNoTrades) {
    return markets;
  }

  console.warn(`[MarketFinder] Bitquery returned 0 trades for all ${markets.length} markets - returning empty to avoid wasting API credits on individual queries`);
  return [];
}
...
export async function discoverMarketsByAsset(asset, timeframe, startTime, endTime, options = {}) {
  ...
  try {
    ...
    const tokenIds = extractTokenIds(marketsNeedingFetch);
    console.log(`[MarketFinder] ${tokenIds.length} unique token IDs to query (${marketsNeedingFetch.length} markets)`);
    ...
    if (tokenIds.length > 0) {
      if (dayRange <= MAX_DAYS_SINGLE_QUERY) {
        ...
        trades = await fetchBitqueryTradesChunked(tokenIds, startTime, endTime, MAX_DAYS_SINGLE_QUERY);
      }
    }

    const fetchedResults = buildMarketObjects(marketsNeedingFetch, trades);
    const coveredResults = buildMarketObjects(marketsAlreadyCovered, [], { returnAllIfNoTrades: true });
