...
export async function discoverMarketsByAsset(asset, timeframe, startTime, endTime) {
  ...
  // Auto-chunk large time ranges to reduce query costs
  const dayRange = Math.ceil((endTime - startTime) / (1000 * 60 * 60 * 24));
  const MAX_DAYS_SINGLE_QUERY = 7;

  if (dayRange > MAX_DAYS_SINGLE_QUERY) {
    console.log(`[MarketFinder] Large time range (${dayRange} days) detected. Auto-chunking into ${MAX_DAYS_SINGLE_QUERY}-day batches...`);
    return batchDiscoverMarkets(asset, timeframe, startTime, endTime, MAX_DAYS_SINGLE_QUERY);
  }
  ...
  try {
    ...
    // Extract all token IDs from filtered markets
    let tokenIds = [];
    ...
    console.log(`[MarketFinder] Fetching Bitquery trade data for ${tokenIds.length} unique token IDs...`);

    // OPTIMIZATION: Use token-filtered query instead of fetching all trades
    let trades = [];
    try {
      if (tokenIds.length > 0) {
        trades = await bitqueryClient.queryAllPolymarketTradesByTokens(
          startTime.toISOString(),
          endTime.toISOString(),
          tokenIds,
          5000  // Reduced from 10000
        );
        console.log(`[MarketFinder] Fetched ${trades.length} trades for ${tokenIds.length} token IDs from Bitquery`);
      } else {
        console.warn('[MarketFinder] No token IDs found in markets, skipping trade data fetch');
      }
    } catch (error) {
      if (error.code === 'QUOTA_EXCEEDED') {
        console.warn('[MarketFinder] Bitquery quota exceeded. Returning markets without trade data.');
        console.warn('[MarketFinder] Markets will still be available but without historical prices.');
        // Continue with empty trades array - markets will be returned without trade data
      } else {
        console.error('[MarketFinder] Error fetching Bitquery trades:', error.message);
        throw error;
      }
    }
