...
async function fetchGammaMarkets(asset, startTime) {
  ...
  const filtered = allMarkets.filter(market => {
    ...
    return market.clobTokenIds && market.clobTokenIds !== '[]';
  });

  console.log(`[MarketFinder] Found ${filtered.length} ${asset} markets after keyword filter`);
  return filtered;
}
 ...
 */
function buildMarketObjects(gammaMarkets, trades) {
...
export async function discoverMarketsByAsset(asset, timeframe, startTime, endTime) {
  ...
  try {
    ...
    const tokenIds = extractTokenIds(gammaMarkets);
    console.log(`[MarketFinder] ${tokenIds.length} unique token IDs to query`);
    ...
    return buildMarketObjects(gammaMarkets, trades);
  } catch (error) {
...
export async function batchDiscoverMarkets(asset, timeframe, startTime, endTime, chunkSizeDays = 7) {
  return discoverMarketsByAsset(asset, timeframe, startTime, endTime);
}
