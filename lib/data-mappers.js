...
export function calculatePriceFromOrderFilled(makerAmountStr, takerAmountStr) {
  ...
  if (price < 0 || price > 1) {
    console.warn(`[DataMapper] Unusual price calculated: ${price} (USDC: ${usdcPaid}, Tokens: ${tokensReceived})`);
  }
...
export function transformOrderFilledToSnapshot(event, conditionId, tokenMapping = {}) {
  try {
    ...
    // Get and round timestamp
    const rawTimestamp = event.Block.Timestamp || Math.floor(new Date(event.Block.Time).getTime() / 1000);
    ...
    console.error('[DataMapper] Failed to transform OrderFilled event:', error);
    console.error('[DataMapper] Event data:', JSON.stringify(event, null, 2));
    return null;
  }
}
...
export function transformConditionPreparationToMarket(event) {
  try {
    ...
    const timestamp = event.Block.Timestamp || Math.floor(new Date(event.Block.Time).getTime() / 1000);
